...
class Network(object):
  ...
  def _proposal_target_layer(self, rois, roi_scores, name):
    with tf.variable_scope(name) as scope:
      rois, roi_scores, labels, bbox_targets, bbox_second_targets, bbox_second_mask, bbox_inside_weights, \
      bbox_outside_weights, train_with_rep_gt = tf.py_func(
  ...
  def _rep_gt_loss(self, bbox_pred, bbox_second_targets, mask):
      ...
      decoded_second_target_boxes = bbox_transform_inv_tf(boxes2, bbox_second_targets_deltas)
      decoded_second_target_boxes = tf.boolean_mask(decoded_second_target_boxes, mask)
      decoded_second_target_boxes = tf.reshape(decoded_second_target_boxes, [-1, 4])
  ...
  def _add_losses(self, sigma_rpn=3.0):
    with tf.variable_scope('LOSS_' + self._tag) as scope:
      ...
      bbox_second_targets = self._proposal_targets['bbox_second_targets']
      mask = self._proposal_targets['bbox_second_mask']
      bbox_inside_weights = self._proposal_targets['bbox_inside_weights']
      bbox_outside_weights = self._proposal_targets['bbox_outside_weights']
      loss_box = self._smooth_l1_loss(bbox_pred, bbox_targets, bbox_inside_weights, bbox_outside_weights)
      rep_gt_loss, self.G = tf.cond(self._etc['train_with_rep_gt'],
                            lambda: self._rep_gt_loss(bbox_pred, bbox_second_targets, mask),
                            lambda: zerof2())
      ...
      self._losses['rep_gt_loss'] = rep_gt_loss

      loss = cross_entropy + loss_box + rpn_cross_entropy + rpn_loss_box + rep_gt_loss
  ...
  def train_step(self, sess, blobs, train_op):
    feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                 self._gt_boxes: blobs['gt_boxes']}
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, rep_gt_loss, loss, cls_prob_debug,bbox_pred_debug,rois_debug, G_debug, _ = sess.run([self._losses["rpn_cross_entropy"],
                                                                        self._losses['rpn_loss_box'],
                                                                        self._losses['cross_entropy'],
                                                                        self._losses['loss_box'],
                                                                        self._losses['rep_gt_loss'],
  ...
  def train_step_with_summary(self, sess, blobs, train_op):
    feed_dict = {self._image: blobs['data'], self._im_info: blobs['im_info'],
                 self._gt_boxes: blobs['gt_boxes']}
    rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, rep_gt_loss, loss, summary, _ = sess.run([self._losses["rpn_cross_entropy"],
                                                                                 self._losses['rpn_loss_box'],
                                                                                 self._losses['cross_entropy'],
                                                                                 self._losses['loss_box'],
                                                                                  self._losses['rep_gt_loss'],
                                                                                 ...
                                                                                 train_op],
                                                                                feed_dict=feed_dict)
    return rpn_loss_cls, rpn_loss_box, loss_cls, loss_box, rep_gt_loss, loss, summary
