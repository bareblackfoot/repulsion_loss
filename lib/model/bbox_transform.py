...
def bbox_transform_inv_tf(boxes, deltas):
  ...
  pred_ctr_x = tf.add(tf.multiply(dx, tf.expand_dims(widths,1)), tf.expand_dims(ctr_x,1))
  pred_ctr_y = tf.add(tf.multiply(dy, tf.expand_dims(heights,1)), tf.expand_dims(ctr_y,1))
  pred_w = tf.multiply(tf.exp(dw), tf.expand_dims(widths,1))
  pred_h = tf.multiply(tf.exp(dh), tf.expand_dims(heights,1))
  ...
  return tf.reshape(tf.stack([pred_boxes0, pred_boxes1, pred_boxes2, pred_boxes3], axis=2), [256,-1])
